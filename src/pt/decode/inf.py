...
def parse_fall_item(segments: list[bytes]) -> PTServerFallItem:
	...
	fall = PTServerFallItem(percent=atoi(segments[0]))
	rest = segments[1:]
	...
	if rest and rest[0] == MONSTER_DROP_NONE:
		...
		fall.kind = "gold"
		fall.gold_min = atoi(rest[1]) if len(rest) > 1 else 0
		fall.gold_max = atoi(rest[2]) if len(rest) > 2 else fall.gold_min
	else:
		fall.kind = "item"
	...
	with open(path, "rb") as f:
		for line in f.readlines():
			...
			match key:
				...
				case INF.Level:
					monster.level = atoi(segments[0])
				case INF.wPlayClass:
					monster.is_boss = True
				case INF.wPlayClass2:
					monster.rank = atoi(segments[0])
				case INF.Size:
					# *모델크기: atof * fONE, 1.0 reset to 0
					monster.model_scale = atof(segments[0])
				case INF.SizeLevel:
				...
				case INF.Move_Speed | INF.MoveSpeed:
					...
					monster.move_speed = atof(segments[0])
				case INF.MoveType:
					# *이동타입: parsed but discarded by the engine
					monster.move_type = atoi(segments[0])
				case INF.MoveRange:
					# *이동범위: atof * fONE (default 64 * fONE)
					monster.move_range = atof(segments[0])
				case INF.Attack_Damage:
					monster.attack_damage = [atoi(v) for v in segments[:2]]
				case INF.Attack_Speed:
					# atof * fONE; second value overwrites the first in the
					# engine (fileread.cpp:4696-4705)
					monster.attack_speed = atof(segments[0])
				case INF.Shooting_Range:
					monster.shooting_range = atoi(segments[0])
				case INF.Attack_Rating:
					monster.attack_rating = atoi(segments[0])
				case INF.Defence:
					monster.defence = atoi(segments[0])
				case INF.Absorption:
					# stray "5%" unit suffixes appear in a few older files; C atoi
					# stops at the '%', so atoi alone reproduces the engine value
					monster.absorption = atoi(segments[0])
				case INF.Chance_Block:
					# stray "6%" unit suffixes appear in a few older files
					monster.chance_block = atoi(segments[0])
				case INF.Life | INF.Life2:
					# single value, copied into both Life slots
					monster.life = atoi(segments[0])
				case INF.Resistance_sITEMINFO_BIONIC:
					monster.resistances["bionic"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_WATER:
					monster.resistances["water"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_LIGHTING:
					monster.resistances["lighting"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_ICE:
					monster.resistances["ice"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_WIND:
					monster.resistances["wind"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_EARTH:
					monster.resistances["earth"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_FIRE:
					monster.resistances["fire"] = atoi(segments[0])
				case INF.Resistance_sITEMINFO_POISON:
					# the INF parser atofes poison resistance then narrows to short
					# (fileread.cpp:4784), unlike the item-table atoi (:3539)
					monster.resistances["poison"] = int(atof(segments[0]))
				case INF.Resistance_sITEMINFO_MAGIC:
					monster.resistances["magic"] = atoi(segments[0])
				case INF.Real_Sight:
					# stored as-is; the engine squares it for the radius test
					monster.sight = atoi(segments[0])
				case INF.ArrowPosi:
					monster.arrow_position = [atoi(v) for v in segments[:2]]
				case INF.SkillDamage:
					monster.skill_damage = [atoi(v) for v in segments[:2]]
				case INF.SkillDistance:
					monster.skill_distance = atoi(segments[0])
				case INF.SkillRange:
					monster.skill_range = atoi(segments[0])
				case INF.SkillRating:
					monster.skill_rating = atoi(segments[0])
				case INF.SkillCurse:
					monster.skill_curse = atoi(segments[0])
				case INF.ActiveHour:
				...
				case INF.GenerateGroup:
					monster.generate_group = [atoi(v) for v in segments[:2]]
				case INF.IQ:
					monster.iq = atoi(segments[0])
				case INF.ClassCode:
					monster.class_code = atoi(segments[0])
				case INF.DamageStunPers | INF.DamageStunPers2:
					monster.damage_stun_percent = atoi(segments[0])
				case INF.Nature:
					if segments[0] == MONSTER_NATURE_GOOD:
						...
						monster.nature = "evil"
				case INF.EventCode:
					monster.event_code = atoi(segments[0])
				case INF.EventInfo:
					monster.event_info = atoi(segments[0])
				...
				case INF.SpAttackPercetage:
					...
					monster.special_attack_percent = atoi(segments[0])
				case INF.Type:
				...
				case INF.Exp:
					monster.exp = atoi(segments[0])
				case INF.PotionCount:
					monster.potion_count = atoi(segments[0])
				case INF.PotionPercent:
					monster.potion_percent = atoi(segments[0])
				case INF.AllSeeItem:
					monster.all_see_item = True
				case INF.FallItemMax:
					monster.fall_item_max = atoi(segments[0])
				case INF.FallItems:
				...
				case INF.ExtraGold:
					...
					if len(segments) >= 2:
						monster.fall_items.append(PTServerFallItem(
							kind = "gold",
							percent = atoi(segments[2]) if len(segments) > 2 else 0,
							gold_min = atoi(segments[0]),
							gold_max = atoi(segments[1])
						))
